...
$db=new PDO('mysql:host='.$dbhost.';dbname='.$dbname,$dbuser,$dbpwd,array(PDO::MYSQL_ATTR_INIT_COMMAND=>'SET NAMES utf8',PDO::ATTR_ERRMODE=>PDO::ERRMODE_WARNING));
   if(isset($_POST['liste'])){
    $r=array($_POST['liste']);
    $sql='INSERT INTO liste(nomListe) VALUES(?)';
    $req=$db->prepare($sql);
    $req->execute($r);
    if ($req) {
        header("location:ListAudience.php");
            }
  
}
  $requete=$db->query("SELECT * FROM liste");
 ?>
...
<form  method="POST" align="center" action="" style="margin-top:50px;margin-left: 100px;" enctype="multipart/form-data">

                    <table  width="500">
                        <tr>
                            <td valign="top">
                            <label for="liste">nomListe</label>
                            </td>
                            <td valign="top">
                            <input  type="text" name="liste" maxlength="50" size="30" value="">
                            </td>
                        </tr>
                         <tr>
                                        ...
                                        <?php 
                                            while ($result=$requete->fetch()) {
                                              ?>
                                               <tbody>
                                                <tr>
                                                    <td><?= $result['idliste']?></td>
                                                    <td><?= $result['nomListe']?></td>
                                                </tr>                                             
                                            </tbody>
                                              <?php
                                            }
